    ...
    >
      <div v-if="syncDialog.server" class="sync-info">
        ...
        <el-form :model="syncDialog.config" label-width="120px">
          ...
          <!-- 下钻深度已隐藏，使用默认值 5 -->
          <el-form-item v-show="false" label="下钻深度">
          ...
          <div class="stat-card">
            ...
            <div class="stat-info">
              <div class="stat-values">
                ...
                <span class="stat-sep">/</span>
                <span class="stat-total">{{ currentProgressTask.maxMembers || '-' }}</span>
              </div>
              <div class="stat-label">已采集 / 采集上限</div>
            </div>
          </div>
  ...
  try {
    ...
    if (Array.isArray(servers)) {
      ...
      for (const s of servers) {
        if (s.discordAccountId && !accountMap.has(s.discordAccountId)) {
          accountMap.set(s.discordAccountId, {
            id: s.discordAccountId,
            name: s.discordAccountName || `账号${s.discordAccountId}`
          })
        }
      }
      accountOptions.value = Array.from(accountMap.values())
    }
  } catch (e) {
