  ...
  <div class="accounts-page">
    <div class="page-header">
      ...
      <div class="header-actions">
        <el-button type="success" @click="openBatchImport"><el-icon><Upload /></el-icon><span>批量导入账号</span></el-button>
        <el-button type="primary" @click="openAddAccount"><el-icon><Plus /></el-icon><span>手工添加</span></el-button>
      </div>
    </div>
    ...
    <div class="page-body">
      <div class="filter-bar">
        <div class="stat-cards">
          <div class="stat-card stat-total"><div class="stat-value">{{ allAccounts.length }}</div><div class="stat-label">账号总数</div></div>
          <div class="stat-card stat-active"><div class="stat-value">{{ allAccounts.filter(a => a.status === 'ACTIVE').length }}</div><div class="stat-label">正常</div></div>
          <div class="stat-card stat-inactive"><div class="stat-value">{{ allAccounts.filter(a => a.status === 'INACTIVE').length }}</div><div class="stat-label">已停用</div></div>
        </div>
        <div class="filter-controls">
          <el-input v-model="filters.keyword" size="default" placeholder="搜索账号、邮箱、备注或ID" :prefix-icon="Search" clearable style="width: 320px;" @keyup.enter="fetchAccounts" />
          ...
          <el-button type="primary" @click="fetchAccounts"><el-icon><Search /></el-icon> 搜索</el-button>
      ...
      <el-table v-loading="loading" :data="filteredAccounts" stripe style="width: 100%;" class="accounts-table">
        ...
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="{ row }">
            <el-button v-if="row.accountType === 'USER'" size="small" type="primary" link @click="syncAccount(row)"><el-icon><Refresh /></el-icon> 同步</el-button>
            <el-button size="small" link type="warning" @click="openEdit(row)"><el-icon><Edit /></el-icon> 编辑</el-button>
            <el-button size="small" link type="danger" @click="removeAccount(row)"><el-icon><Delete /></el-icon> 删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="botDialog.visible" :title="botDialog.editId ? '编辑账号' : '手工添加账号'" width="480px" @close="resetBotDialog">
      <el-form :model="botDialog.form" label-width="100px">
        <el-form-item label="账号名称" required><el-input v-model="botDialog.form.nickname" placeholder="请输入账号名称" /></el-form-item>
        <el-form-item label="Token" required><el-input v-model="botDialog.form.token" type="password" show-password placeholder="请输入 Token" /></el-form-item>
        ...
        <el-form-item label="邮箱"><el-input v-model="botDialog.form.email" placeholder="邮箱（选填）" /></el-form-item>
        <el-form-item label="备注"><el-input v-model="botDialog.form.remark" type="textarea" :rows="2" placeholder="备注（选填）" /></el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="batchDialog.resultVisible" title="导入结果" width="720px" :close-on-click-modal="true">
      <div style="margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center;">
        <div style="display: flex; gap: 16px; font-size: 14px;">
          ...
          <span style="color: var(--color-green)">成功: <b>{{ batchDialog.resultSuccess }}</b></span>
          <span style="color: var(--color-red)">失败: <b>{{ batchDialog.resultFailed }}</b></span>
        </div>
        <el-button v-if="batchDialog.resultFailed > 0" type="warning" size="small" @click="exportFailedAccounts">
          <el-icon><Download /></el-icon>导出失败账号
        </el-button>
      </div>
      <el-table :data="batchDialog.results" max-height="400" size="small" border>
        ...
        <el-table-column label="密码" width="120">
          <template #default="{ row }">
            <span v-if="!row.success && row.password" style="color: var(--color-red); font-family: monospace;">{{ row.password }}</span>
        ...
        <el-table-column prop="message" label="详情" show-overflow-tooltip>
          <template #default="{ row }">
            <span :style="{ color: row.success ? 'var(--color-text-2)' : 'var(--color-red)' }">{{ row.message }}</span>
          </template>
        </el-table-column>
      </el-table>
      <template #footer>
...
import { Plus, Upload, Refresh, Edit, Delete, Search, OfficeBuilding, Download } from '@element-plus/icons-vue'
import { listAccounts, createAccount, updateAccount, deleteAccount, batchImport, syncAccountRelationships, listMerchants } from '@/api'
...
async function fetchMerchants() {
  ...
  try { const res = await listMerchants(); merchants.value = Array.isArray(res) ? res : (res?.data || []) } catch (e) {} finally { merchantsLoading.value = false }
}
...
async function fetchAccounts() {
  ...
  } catch (e) { allAccounts.value = [] } finally { loading.value = false }
}
...
async function syncAccount(acc) { try { await syncAccountRelationships(acc.id); ElMessage.success('同步请求已发送'); await fetchAccounts() } catch (e) {} }

const botDialog = reactive({ visible: false, editId: null, saving: false, form: { nickname: '', token: '', accountType: 'BOT', email: '', remark: '', merchantId: null } })
...
async function saveBot() {
  ...
  try {
    if (botDialog.editId) { await updateAccount(botDialog.editId, { nickname: botDialog.form.nickname, token: botDialog.form.token, remark: botDialog.form.remark, merchantId: botDialog.form.merchantId }); ElMessage.success('已更新') }
    else { await createAccount({ name: botDialog.form.nickname, token: botDialog.form.token, accountType: botDialog.form.accountType, email: botDialog.form.email, remark: botDialog.form.remark, merchantId: botDialog.form.merchantId }); ElMessage.success('已添加') }
    botDialog.visible = false; await fetchAccounts()
  } finally { botDialog.saving = false }
}

const batchDialog = reactive({ visible: false, saving: false, text: '', resultVisible: false, results: [], resultTotal: 0, resultSuccess: 0, resultFailed: 0 })
function openBatchImport() { batchDialog.visible = true; batchDialog.text = ''; batchDialog.resultVisible = false; batchDialog.results = [] }
function resetBatchDialog() { batchDialog.text = ''; batchDialog.saving = false; batchDialog.resultVisible = false; batchDialog.results = [] }

async function runBatchImport() {
  const lines = batchDialog.text.split(/\r?\n/).map(s => s.trim()).filter(Boolean)
  if (lines.length === 0) { ElMessage.warning('请输入至少一行'); return }
  const items = []
  for (const line of lines) { const [email, password] = line.split('|').map(s => s?.trim()); if (!email || !password) continue; items.push({ email, password }) }
  ...
  try {
    ...
    batchDialog.resultTotal = total; batchDialog.resultSuccess = ok; batchDialog.resultFailed = fail; batchDialog.results = results
    if (fail === 0 && total > 0) { ElMessage.success(`导入完成，成功 ${ok} / ${total}`); batchDialog.visible = false }
    else { batchDialog.resultVisible = true; if (ok > 0 && fail > 0) ElMessage.warning(`部分导入成功，成功 ${ok}，失败 ${fail}`); else ElMessage.error(`全部导入失败 ${fail} / ${total}`) }
    if (ok > 0) { batchDialog.visible = false; await fetchAccounts() }
  } catch (e) {
    batchDialog.resultTotal = items.length; batchDialog.resultSuccess = 0; batchDialog.resultFailed = items.length
    batchDialog.results = items.map(i => ({ email: i.email, password: i.password, success: false, message: '请求失败: ' + (e?.message || e), accountName: null }))
    batchDialog.resultVisible = true; ElMessage.error('请求失败，已生成失败列表')
  } finally { batchDialog.saving = false }
}

function exportFailedAccounts() {
  const failed = batchDialog.results.filter(r => !r.success)
  if (failed.length === 0) { ElMessage.warning('没有失败的账号'); return }
...
onMounted(() => { fetchAccounts(); fetchMerchants() })
watch(() => filters.status, () => { fetchAccounts() })
function getMerchantName(merchantId) { if (!merchantId) return ''; const m = merchants.value.find(x => x.id === merchantId); return m ? m.name : '' }

let searchTimer = null
watch(() => filters.keyword, () => { clearTimeout(searchTimer); searchTimer = setTimeout(() => { fetchAccounts() }, 400) })
</script>

<style scoped>
.accounts-page { width: 100%; height: 100%; display: flex; flex-direction: column; overflow: hidden; }
.page-header { display: flex; align-items: center; justify-content: space-between; padding: 20px 24px 16px; border-bottom: 1px solid var(--color-border); background: var(--color-bg-2); }
...
.stat-card { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 10px 20px; border-radius: 8px; min-width: 120px; background: var(--color-bg-3); }
...
.account-avatar-cell { flex-shrink: 0; background: linear-gradient(135deg, var(--color-primary), var(--color-pink)); color: #fff; font-weight: 600; }
